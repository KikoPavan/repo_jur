...
**Nota de encerramento (2026-08-14):** a implementação foi autorizada por `/goal` explícito e concluída exatamente conforme a recomendação acima (Candidato A, sem desvios). Ver `proposal.md`, seção "Implementação", para o relato completo (helper `_strip_internal_ocr_markers`, testes, validação com dados reais, delta spec e blast radius confirmado). Todas as premissas deste diagnóstico se confirmaram na implementação: nenhum efeito colateral inesperado foi descoberto (ao contrário de `fix-ocr-rotated-text-fragmentation`, cujo ponto de intervenção original precisou ser corrigido durante a implementação).
