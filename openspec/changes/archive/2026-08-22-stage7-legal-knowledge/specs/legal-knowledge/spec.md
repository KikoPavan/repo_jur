...
## Purpose
Define the Stage 7 Legal Knowledge Pipeline — the `legal_knowledge` branch of the closed Stage 6 Domain Router — composed of the Legal Semantic Review / Enrichment seam and the Legal Producer. The capability consumes only conformant Phase 1 artifacts (literal Markdown and technical conversion report recording Quality Gate outcome `PASS` or `PASS_WITH_WARNINGS`, routed to `legal_knowledge`) strictly read-only, produces structured review patches that preserve every original word, routes ambiguity to `REVIEW_REQUIRED`, renders Legal OKF concept candidates per the Legal OKF Profile, resolves identity/provenance and duplicate acts conservatively, applies the lifecycle/field-ownership merge, validates OKF + profile + cardinality, and publishes atomically into `repo_jur/bundle/` exclusively through `guard_legal_bundle_write` — without mutating Phase 1 artifacts, without silent semantic/LLM classification, without Judicial-Process behavior, and without automatic `verified`/`status`/versioning.
