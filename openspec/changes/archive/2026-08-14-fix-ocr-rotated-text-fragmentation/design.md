...
## Implementação (2026-08-12) — refinamento do ponto de intervenção

A ETAPA 5 recomendou o Ponto C (pós-processar o `raw_content` por página, imediatamente após obtê-lo do OCR, antes da composição do documento). A implementação (ver `proposal.md`, seção "Implementação", para o relato completo) descobriu, com dados 100% reais e sem nenhuma chamada de OCR nova, que esse ponto exato causava um efeito colateral: a reconstrução geométrica — idêntica nas 4 páginas do Testamento, por ser o mesmo carimbo e-SAJ — tornava-se a nova "última linha" de cada página e era removida pelo cleaner já existente `remove_repetitive_margins` (`cleaner.py`), que remove verbatim linhas finais idênticas entre ≥60% das páginas; em cascata, o próprio marcador `[End OCR]*` (também idêntico nas 4 páginas) era removido em seguida, violando a restrição de que sua presença/remoção não deveria ser afetada por esta mudança.

**Ponto de intervenção final:** a mesma lógica de detecção/substituição (posição estrutural após `[End OCR]*` + corroboração geométrica), mas aplicada **depois** de `remove_repetitive_margins` já ter rodado, sobre o documento já composto, por região `[[Pág. N]]`. A geometria de cada página (`_non_horizontal_line_texts`) é calculada e armazenada durante o loop por página já existente em `convert_document` — antes dos arquivos temporários de página serem descartados pelo gerenciador de contexto `isolated_page_workspace` — e reaproveitada nesse ponto posterior, sem reabrir nenhum arquivo. Isso preserva exatamente o comportamento hoje existente de `remove_repetitive_margins` (incluindo sua não-remoção do marcador) e não exige nenhuma alteração em `cleaner.py`. Validado com os mesmos dados reais: `[End OCR]*` sobrevive nas 4 páginas, a reconstrução aparece corretamente, zero ruído remanescente.

Esse refinamento não muda a causa raiz, o critério (Candidato E) nem o blast radius já documentados acima — apenas o estágio exato, dentro de `convert_document`, em que o critério já especificado é avaliado e aplicado.
