...
- [ ] 1.7 Adicionar teste de controle próximo ao limiar tipográfico: um bloco com tamanho 19.9pt não aciona o gate; um bloco com tamanho exatamente 20.0pt aciona. Parcialmente coberto nesta subtarefa (`test_recompose_native_paragraphs_gate_threshold_boundary`, apenas o parâmetro booleano de `recompose_native_paragraphs`); o teste do limiar de fonte real (19.9pt vs 20.0pt, medido via PyMuPDF em `converter.py`) será adicionado na Subtarefa 2, já que a detecção de tamanho de fonte ainda não existe no pipeline.
