...
**Diagnóstico concluído em duas rodadas, implementação BLOQUEADA**: três critérios geométricos candidatos foram desenhados e validados empiricamente contra o corpus real (não apenas testes sintéticos) — mesmo-bloco-apenas; recuo x0 obrigatório entre blocos; e uma versão refinada do recuo x0 (com bloqueio de mesma-linha-visual, isenção para títulos após marcador bare, e colapso de margem) que chegou a resolver *todos* os casos do Código Civil, incluindo os que a primeira versão havia quebrado. Mesmo assim, a versão refinada foi descartada: validada contra o corpus inteiro, ela fragmenta gravemente parágrafos inteiros de fundamentação jurídica em `Inf0024E.pdf` e `AINTARESP_1462304-PA.pdf` (um parágrafo por frase), porque esses dois documentos não seguem a convenção de recuo-na-primeira-linha do Código Civil — uma regressão maior que o problema original. Nenhum critério seguro foi encontrado — conforme a diretriz explícita da tarefa ("Se não encontrar um critério que separe com segurança Papel/Nome dos casos R01, PARE e informe. Não implemente heurística ampla."), esta mudança permanece em diagnóstico, sem implementação, aguardando decisão humana sobre como prosseguir.
