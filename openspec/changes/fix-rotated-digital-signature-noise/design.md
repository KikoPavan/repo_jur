...
Nenhum arquivo de `src/`, `tests/`, `output/`, `logs/` ou do corpus canônico foi alterado ou reconvertido durante este diagnóstico. Todas as inspeções usaram cópias de página isoladas em diretórios temporários (`tempfile.mkdtemp()`), nunca sobrescrevendo `input/` nem `output/`. `git status --short` ao final desta mudança lista apenas os três arquivos novos de `openspec/changes/fix-rotated-digital-signature-noise/` (`proposal.md`, `design.md`, `tasks.md`).
